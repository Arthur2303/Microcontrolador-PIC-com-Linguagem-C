...
void main () {
   ...
   setup_adc_ports( AN0 );//Específica a porta analógica que será utilizada comco inputs.
   set_adc_channel(0);//Determinando o canal ativo.
   
   while(true) {
      valor = read_adc();//Lê-se a porta ativada pela função set_adc_channel e guarda em valor
